...
def payload_optin():
    payload = copy.deepcopy(base_payload)
    payload["entry"][0]["messaging"][0]["optin"] = {"ref": "PASS_THROUGH_PARAM"}
    return payload
...
def test_account_linking(messenger, payload_account_linking):
    ...
    mock_account_linking.assert_called_with(
        payload_account_linking["entry"][0]["messaging"][0]
    )
...
def test_upload_attachment(messenger, monkeypatch):
    mock = Mock()
    mock.return_value = {"attachment_id": 12345}
