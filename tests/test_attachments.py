...
class TestAttachments:
    ...
    def test_audio(self):
        ...
        expected = {
            "attachment": {
                "type": "audio",
                "payload": {"url": "http://facebook.com/audio.mp3"},
            }
        }
        assert expected == res.to_dict()
    ...
    def test_video(self):
        ...
        expected = {
            "attachment": {
                "type": "video",
                "payload": {"url": "http://facebook.com/video.mp4"},
            }
        }
        assert expected == res.to_dict()
    ...
    def test_file(self):
        ...
        expected = {
            "attachment": {
                "type": "file",
                "payload": {"url": "http://facebook.com/file.txt"},
            }
        }
        assert expected == res.to_dict()
    ...
    def test_quick_replies(self):
        ...
        res = attachments.Image(url="http://facebook.com/image.jpg", quick_replies=qrs)
        expected = {
            "attachment": {
                "type": "image",
                "payload": {"url": "http://facebook.com/image.jpg"},
    ...
    def test_quick_replies_throws_error(self):
        with pytest.raises(ValueError) as err:
            attachments.Image(
                url="http://facebook.com/image.jpg", quick_replies="error"
            )
        assert str(err.value) == "quick_replies must be an instance of QuickReplies."

    def test_is_reusable(self):
        res = attachments.Image(url="http://facebook.com/image.jpg", is_reusable=True)
        expected = {
            "attachment": {
                "type": "image",
                "payload": {
