...
class TestThreadSettings:
    ...
    def test_persistent_menu_item_web_url(self):
        res = thread_settings.PersistentMenuItem(
            item_type="web_url", title="Link", url="https://facebook.com"
        )
        expected = {"type": "web_url", "title": "Link", "url": "https://facebook.com"}
    ...
    def test_persistent_menu_item_postback(self):
        res = thread_settings.PersistentMenuItem(
            item_type="postback", title="Link", payload="payload"
        )
    ...
    def test_persistent_menu_nested_item(self):
        ...
        expected = {
            ...
            "title": "Nested",
            "call_to_actions": [
                {"type": "postback", "title": "Link", "payload": "payload"}
            ],
        }
        assert expected == res.to_dict()
    ...
    def test_invalid_item_type(self):
        with pytest.raises(ValueError) as err:
            thread_settings.PersistentMenuItem(
                item_type="invalid", title="Link", payload="payload"
            )
    ...
    def test_invalid_title(self):
        with pytest.raises(ValueError) as err:
            thread_settings.PersistentMenuItem(
                item_type="postback", title="x" * 31, payload="payload"
            )
    ...
    def test_invalid_payload(self):
        with pytest.raises(ValueError) as err:
            thread_settings.PersistentMenuItem(
                item_type="postback", title="Link", payload="x" * 1001
            )
    ...
    def test_too_many_nested_items(self):
        ...
        with pytest.raises(ValueError) as err:
            thread_settings.PersistentMenuItem(
                item_type="nested", title="Nested", nested_items=[item] * 6
            )
    ...
    def test_composer_input_disabled(self):
        ...
        res = thread_settings.PersistentMenu(
            menu_items=[item], composer_input_disabled=True
        )
        profile = thread_settings.MessengerProfile(persistent_menus=[res])
        expected = {
            "persistent_menu": [
