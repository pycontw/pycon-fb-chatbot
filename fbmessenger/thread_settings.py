...
class PersistentMenuItem(object):
    ITEM_TYPES = ["nested", "web_url", "postback"]
    ...
    def __init__(
        ...
        if item_type == "web_url":
            ...
            if (
                ...
                and not webview_height_ratio in WEBVIEW_HEIGHT_RATIOS
            ):
    ...
    def to_dict(self):
        res = {"type": self.item_type, "title": self.title}
...
class PersistentMenu(object):
    ...
    def to_dict(self):
        ...
        if self.menu_items:
            res["call_to_actions"] = [item.to_dict() for item in self.menu_items]
...
class MessengerProfile(object):
    ...
    def to_dict(self):
        ...
        if self.greetings:
            res["greeting"] = [item.to_dict() for item in self.greetings]

        if self.get_started:
            res["get_started"] = self.get_started.to_dict()

        if self.persistent_menus:
            res["persistent_menu"] = [item.to_dict() for item in self.persistent_menus]
